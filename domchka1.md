...
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~$ grep '.*' /etc/passwd | cut -d: -f1 | sort
_apt
avahi
backup
bin
...
```
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 ./banner.sh "narek1"
+--------+
| narek1 |
+--------+
```
...
```
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 grep -oE '\b[a-zA-Z_][a-zA-Z0-9_]*\b' hello.c | grep -vE '\b(int|void|return|if|else|for|while|include|stdio)\b' | sort | uniq
h
hello
main
n
...
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 ./reg.sh banner.sh
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 ls /usr/local/bin
banner.sh  ngrok
```
...
```bash
narek1@narek1-System-Product-Name:~/Desktop/confupr$ ./6.sh 
...
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 ./tar.sh "test" "py"
...
```
```bash
narek1@narek1-System-Product-Name:~/Desktop/confupr$ ./6.sh test.c test_new.c
...
```
```bash
narek1@narek1-Strix-GL504GW-GL504GW:~/domchka1 ./10.sh test
test/3.js
test/1.py
test/1ff
test/2.py
